...
import { getAdminCategoryById, updateAdminCategory, getAdminCategories } from '../../../services/api';
...
function AdminCategoryEdit() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Loại trừ chính danh mục này khỏi danh sách chọn cha để tránh lỗi tự liên kết
        const filteredCategories = (categoriesRes.data.categories || []).filter(c => c.id !== parseInt(id));
  ...
  const onFinish = async (values) => {
    ...
    if (values.parentId !== undefined) {
      data.append('parentId', values.parentId !== null ? values.parentId : '');
    }
        ...
        <Card>
          <Form form={form} layout="vertical" onFinish={onFinish} onFinishFailed={() => message.error("Vui lòng kiểm tra lại các trường thông tin bị thiếu hoặc sai định dạng!")}>
            ...
            <Form.Item label="Danh mục cha" name="parentId">
              <Select placeholder="-- Chọn danh mục cha (tuỳ chọn) --" allowClear size="large">
                {categories.map(cat => (
